...
import { useScroll, ScrollControls, Float, ContactShadows, Environment, MeshReflectorMaterial, Text } from '@react-three/drei';
import * as THREE from 'three';

const LeatherMaterial = ({ color = "#1A1917" }) => (
...
const Wallet = () => {
  ...
  useFrame((state) => {
    ...
    // Wallet slides in from left (0 to 0.4)
    ...
    if (ref.current) {
        ...
        ref.current.position.z = s > 0.6 ? (s - 0.6) * -5 : 0; // Settling into box
    }
  });
...
const Notebook = () => {
    ...
    useFrame((state) => {
      const s = scroll.offset;
      // Notebook slides in from right (0.1 to 0.5)
...
const Pen = () => {
    ...
    useFrame((state) => {
      const s = scroll.offset;
      // Pen rises from bottom (0.2 to 0.6)
